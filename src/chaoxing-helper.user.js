...
// @updateURL   https://cdn.jsdelivr.net/gh/Cubxx/tampermonkey-script/src/chaoxing-helper.user.js
// @downloadURL https://cdn.jsdelivr.net/gh/Cubxx/tampermonkey-script/src/chaoxing-helper.user.js
// @grant       none
// ==/UserScript==
...
(function () {
  ...
  tm.matchURL(
    ...
    [
      ...
      () => {
        window.addEventListener('load', () => {
          /** 右侧菜单所有未完成小节 */
          const list = $$('.posCatalog_select').filter(
    ...
    //视频
    [
      /ananas\/modules\/video\/index.html\?v=/,
      () => {
        function main() {
          if (!top) return;
          const videoDom = $('video#video_html5_api');
          if (!videoDom) return;
          ...
          //隐藏弹窗
          const el = $('.ans-timelineobjects');
          el?.observe(() => el.hide(), { childList: true, subtree: true });
          //完成任务点后下一节
          const btn = parent['tm'].$('.ans-job-icon');
            ...
            text.split('\n\n').map((e) => {
              ...
              const tasks = {
                ...
                6: () => {
                  ...
                  const options = (ans.length > 1 ? ans.split('') : [ans]).map(
                    (e) => ({ A, B, C, D })[e].slice(2),
                  );
    ...
    [
      /mycourse\/stu/,
      () => {
        $(`#boxscrollleft .stuNavigationList ul li`)
          ?.$('a[title=章节]')
          ?.el.click();
      },
    ],
    ...
    [
      /ueditorupload\/read/,
      () => {
        $$(/** @type {'iframe'} */ ('.mainCon,#reader,#reader>iframe')).forEach(
          (e) =>
            e.set({
              style: {
                width: '90%',
    ...
    [
      /ananas\/modules\/pdf/,
      () => {
        const dom = $('iframe#docContainer');
        if (!dom) return;
        dom.set({ style: { height: '100%' } });
        dom.observe(
          () => {
            $('#img')?.set({ style: { height: '100%' } });
          },
          { childList: true },
        );
      },
