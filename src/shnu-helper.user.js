...
// @updateURL   https://cdn.jsdelivr.net/gh/Cubxx/tampermonkey-script/src/shnu-helper.user.js
// @downloadURL https://cdn.jsdelivr.net/gh/Cubxx/tampermonkey-script/src/shnu-helper.user.js
...
(function () {
  ...
  tm.matchURL(
    ...
    //cas自动登录
    [
      /cas.shnu.edu.cn\/cas\/login\?service=/,
      () => {
        (function submit() {
          const cuid = $('#un'),
            pd = $('#pd'),
    ...
    //教务系统
    [
      /^https\:\/\/course.shnu.edu.cn\/eams\/home.action$/,
      () => {
        setTimeout(() => {
          const grp = $$('.menu > li').filter(
            (e) => e.el.innerText === '公共服务',
          )[0];
          const btn = grp
            ?.$$('li>a')
    ...
    //查找课程
    [
      /course.shnu.edu.cn\/eams\/stdSyllabus!search.action/,
      () => {
        //添加input元素
        ...
        function record() {
          ...
          return doms.map((dom) => {
            ...
            const data = dom.$$('td:not([class])').map((e) => e.el.innerText); //length:10
